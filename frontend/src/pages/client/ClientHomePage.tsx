...
export default function ClientHomePage() {
  ...
  const getStatusVariant = (status: string): "outline" | "secondary" | "default" => {
    switch (status) {
      case "OPEN":
        return "outline";
      case "IN_PROGRESS":
    ...
    <section className="flex-1 flex flex-col space-y-16">
      ...
      <section className="relative py-28 px-6 bg-gradient-to-b from-background to-muted/40 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto mb-10">
            Manage your jobs, review proposals, and collaborate with freelancers seamlessly.
          </p>

          <div className="flex flex-col sm:flex-row justify-center gap-6">
            <Button asChild size="lg" className="gap-2 hover:shadow-2xl hover:-translate-y-1 transition-all">
            ...
            <Button asChild size="lg" variant="outline" className="gap-2 hover:bg-primary hover:text-white transition-all">
              <Link to="/client/jobs">
      ...
      <section className="py-20 px-6 bg-muted/30">
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <p className="text-muted-foreground text-lg">
            Follow these steps to post jobs, review proposals, and successfully complete projects.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {DASHBOARD_STEPS.map((step, index) => (
            <Card key={index} className="border bg-background/70 backdrop-blur-sm hover:shadow-2xl hover:-translate-y-2 transition-all">
      ...
      <section className="px-6 max-w-7xl mx-auto">
        ...
        {jobs.slice(0, 5).map((job) => (
          <Card key={job.id} className="flex justify-between items-center p-4 mb-4">
            <div>
        ...
        {jobs.length === 0 && <p className="text-center text-muted-foreground">No jobs yet. Start by creating a new job!</p>}
      </section>
    </section>
  );
}